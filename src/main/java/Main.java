    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if ("--directory".equals(args[i]) && i + 1 < args.length) {
                ...
                System.out.println(STR."Serving files from directory: \{filesDirectory}");
                break;
            }
        }
        ...
        router.addRoute(GET, "/echo/{name}", new EchoHandler());
        router.addRoute(GET, "/user-agent", new UserAgentHandler());
        if (filesDirectory != null) {
            router.addRoute(GET, "/files/{filename}", new FileHandler(Paths.get(filesDirectory)));
        ...
        try {
            ...
            System.err.println(STR."Failed to start HTTP server: \{e.getMessage()}");
            e.printStackTrace();
        }
    }
}